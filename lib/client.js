...
.cgp-root.cgp-narrow{width:48px!important;padding:7px 6px;cursor:pointer;
...
`
        ...
        function applyNarrow() {
          ...
          if (narrowMode) {
            ...
            // Leaving narrow mode: restore the width and collapsed state the user had before.
        ...
        /** Enter below NARROW_ENTER, leave above NARROW_EXIT (hysteresis avoids flicker at the boundary). */
        function evaluateNarrow() {
          const w = window.innerWidth
          if (narrowDismissed && w >= NARROW_EXIT) narrowDismissed = false
          const should = !narrowDismissed && (narrowMode ? w < NARROW_EXIT : w < NARROW_ENTER)
          if (should !== narrowMode) {
            narrowMode = should
            applyNarrow()
          } else {
            // 视口变化但状态未变：仍要保证浮窗（尤其 48px 竖条）完整落在窗口内
            clampRootIntoView()
          }
        }
        const onWindowResizeClamp = () => clampRootIntoView()
        window.addEventListener('resize', onWindowResizeClamp)
        const narrowObserver = (typeof ResizeObserver === 'function')
          ? new ResizeObserver(() => evaluateNarrow())
          : undefined
        if (narrowObserver) narrowObserver.observe(document.documentElement)
        else window.addEventListener('resize', evaluateNarrow)
        ...
        return () => {
          ...
          else window.removeEventListener('resize', evaluateNarrow)
          root.remove()
        }

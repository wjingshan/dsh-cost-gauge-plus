...
window.__ModuleLoader__.load({
  ...
  factory: () => {
    ...
    /* ---------- 明暗主题 ----------
     * 日间与夜间都使用深色底，只靠色调区分：夜间纯黑（#000），日间冷蓝灰（#1b2029）。
     * 主题以宿主（dsh 界面外观）投影到 DOM 的信号为准：
     *   document.documentElement.style.colorScheme = 'light' | 'dark'（ui-layout 写入）
     *   body[data-ds-dark-theme]（深色时存在）
     * 两者都读不到时（独立打开页面等场景）退回系统 prefers-color-scheme。
     */
    function hostIsDark() {
      if (typeof document === 'undefined') return true
      const de = document.documentElement
      const forced = ((de && de.style && de.style.colorScheme) || '').toLowerCase()
      if (forced.includes('dark') && !forced.includes('light')) return true
      if (forced.includes('light') && !forced.includes('dark')) return false
      if (document.body && document.body.hasAttribute('data-ds-dark-theme')) return true
      return !!(window.matchMedia && matchMedia('(prefers-color-scheme: dark)').matches)
    }
    /** 'night'（纯黑）| 'day'（冷蓝灰深色） */
    function themeMode() {
      return hostIsDark() ? 'night' : 'day'
    }
...
/* 日间（宿主界面浅色）：同样是深色底，只比夜间的纯黑略亮、偏冷蓝灰，一眼可辨 */
.cgp-root[data-cgp-mode="day"]{
  --cgp-bg:#1b2029;--cgp-fg:#eef2f7;--cgp-muted:#98a2b0;
  --cgp-border:rgba(255,255,255,.14);--cgp-input-bg:rgba(255,255,255,.07);
  --cgp-pill-bg:#141821;--cgp-pill-border:rgba(255,255,255,.26);
  --cgp-track:rgba(255,255,255,.13);--cgp-resize:rgba(255,255,255,.38);
  --cgp-flap-bg:#232936;--cgp-flap-fg:#f0f4f9;
  --cgp-disc:rgba(255,255,255,.05);
  --cgp-accent:#8fbde4;box-shadow:0 12px 30px rgba(0,0,0,.45)}
...
/* 设置面板底部：版本号（小字、次要色） */
.cgp-version{margin-top:9px;padding-top:7px;border-top:1px solid var(--cgp-border);
  font-size:10px;line-height:1.3;color:var(--cgp-muted);text-align:center;letter-spacing:.2px;
  font-variant-numeric:tabular-nums;user-select:text;-webkit-user-select:text;cursor:text}
...
<div class="cgp-compact">
  ...
  <div class="cgp-row"><span>${t('balance')}</span><span class="cgp-val cgp-balance2">—</span></div>
          ...
          function renderRing() {
            ...
            const night = hostIsDark()
            const key = `${st.isWeekend()}|${isPM}|${w}|${busy}|${idle}|${night && stars}`
            if (key !== lastRingKey) {
              lastRingKey = key
              const idleC = (stars && night) ? 'url(#cgp-star-grad)' : idle
              buildClockRing(ringEl, st.isWeekend(), isPM, w, busy, idleC)
              if (stopStars) { stopStars(); stopStars = null }
              if (stars && starLayer) {
                // 星空只在夜间点亮：日间同为深色底，但不铺星点，作为日夜的区分特征
                if (night) stopStars = startStars(starLayer, st.isWeekend(), isPM)
          ...
          // 主题切换由外壳统一监听（见 apply 中的 syncThemeMode），这里只需在重建时收尾星空动画
          let stopStars = null
...
<div class="cgp-settings">
  ...
  <div class="cgp-version" title="dsh-cost-gauge-plus"></div>
</div>
...
`
        ...
        // 日间同样深色底：宿主界面浅色 → day（冷蓝灰），深色 → night（纯黑）
        root.dataset.cgpMode = themeMode()
        ...
        const versionEl = root.querySelector('.cgp-version')
        ...
        function render(data) {
          ...
          // 设置面板最下方：当前版本（由宿主 /state 下发，取自 package.json）
          const ver = typeof data.version === 'string' ? data.version.trim() : ''
          if (ver) {
            if (versionEl.dataset.ver !== ver) {
              versionEl.dataset.ver = ver
              versionEl.textContent = 'dsh-cost-gauge-plus v' + ver
            }
        ...
        // ---- 主题跟随：宿主外观（浅色/深色）或系统偏好变化时，切换 day / night 配色 ----
        let themeNow = root.dataset.cgpMode
        function syncThemeMode() {
          const mode = themeMode()
          if (mode === themeNow) return
          themeNow = mode
          root.dataset.cgpMode = mode
          if (lastData) render(lastData)
          tick()
        }
        const themeObserver = typeof MutationObserver === 'function' ? new MutationObserver(syncThemeMode) : null
        if (themeObserver) {
        ...
        const onThemeMq = () => syncThemeMode()
        if (themeMq && themeMq.addEventListener) themeMq.addEventListener('change', onThemeMq)
        ...
        return () => {
          ...
          if (themeMq && themeMq.removeEventListener) themeMq.removeEventListener('change', onThemeMq)
          root.remove()
        }

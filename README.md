...
- **窗口自适应（竖版最小化）**：用 `ResizeObserver` 观察视口宽度，**< 780px** 自动切换为 48px 宽的**竖版最小化**（自上而下：状态灯 → 话费 → 余额 → 模型徽标，均竖排），**点击即展开**；窗口回到 **> 860px**（即 ≥860px）自动退出并恢复进入前的**皮肤与展开/折叠状态**。
...
- 📱 **窗口自适应**：窗口变窄（视口 < 780px）时自动切为 **48px 竖版最小化**（灯 + 话费 + 余额 + 模型徽标），点击展开；窗口恢复（≥860px）自动还原皮肤与展开/折叠态。
- 🔍 **展开 / 缩小两种状态**：

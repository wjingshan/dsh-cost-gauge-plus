...
> 测试1 皮肤在**夜间**将空闲弧显示为深邃星空蓝渐变（#040a3a→#00081e），弧内星点会"亮起→变暗消失→随机换位重现"；日间不铺星点。

## 本次更新（v1.5.3）

- **设置面板底部标注当前版本号**：宿主 `/state` 下发 `version`（读自 `package.json`），设置面板最下方用 10px 次要色小字显示 `dsh-cost-gauge-plus vX.Y.Z`；取不到版本时整行隐藏。

## 本次更新（v1.5.2）

- **日间也走深色底**：以前宿主界面为**浅色（白天）**时浮窗是白底，现在日间改为**冷蓝灰深色（#1b2029）**，与**夜间的纯黑（#000）**保持一档可辨的差异（翻牌窗 / 胶囊 / 文字同步调亮），并保留阴影与发光滤镜。
  - 主题判定改为读宿主投影到 DOM 的信号（`document.documentElement.style.colorScheme` 与 `body[data-ds-dark-theme]`），两者都取不到时才退回系统 `prefers-color-scheme`（独立打开页面等场景）。
  - 用 `MutationObserver` 监听宿主外观切换，**外观一变立即换色，无需刷新页面**。
  - `测试1` 星空皮肤只在**夜间**铺星点，日间不铺（作为日夜区分特征）。
  - 顺带修好浅色外观下部分皮肤（白色描边 / 浅色文字）在白底上几乎不可见的问题。

## 本次更新（v1.5.1）

- **修复位置卡在左上角**：窗口最小化 / 后台标签页里加载时视口宽高为 0，「钳制到窗口内」会把浮窗位置算成 (8,8) 并写进 `localStorage`（之后重启仍卡在左上角）；现在视口为 0 时不做钳制。
...
- 🌗 **日间同样是深色底**：跟随宿主界面外观自动切换，**夜间纯黑**（#000）、**日间冷蓝灰深色**（#1b2029，翻牌窗/胶囊同步调亮），既保持深色又一眼可辨；宿主外观（浅色/深色/跟随系统）一变立即生效，无需刷新。
